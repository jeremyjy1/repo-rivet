...
轻量语义引擎 RivetIndex 通过进程内 Tree-sitter 和会话级 SQLite 增量索引为 C/C++、Python、JavaScript 与 TypeScript 提供文件符号、工作区符号、定义候选、引用候选和语法诊断。统一的只读 `semantic_query` 工具在主 Agent、Plan Mode 和受限子代理中可用；所有结果绑定内容快照与工作区修订，并明确返回查询状态、置信度和降级警告。语法索引不能证明跨模块类型关系时会保留多个候选，文本回退始终标记为低置信度；编译、类型、构建和测试诊断仍通过现有 Verification Runner 执行，不在索引层运行项目代码。首版不提供后台索引、自动重命名、Hover、补全或 LSP 进程管理。
