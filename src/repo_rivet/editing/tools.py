...
class EditFileTool(BaseTool[EditFileArguments]):
    ...
    description = (
        ...
        "Prefer one operation and no more than 80 replacement lines per request. Never replace "
        "an entire large file in one call; apply multiple snapshot-bound edits and reread "
        "between calls."
    )
